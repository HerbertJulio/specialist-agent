...
description: "Create the complete data layer (types + adapter + service) for a resource"
user-invocable: true
...
3. Create in order:
   ...
   d. `services/[resource]-service.ts` -- HTTP only, no try/catch, no transformation, native `fetch`
...
4. Required rules:
   ...
   - Service uses native `fetch` (available in both server and client in SvelteKit)
