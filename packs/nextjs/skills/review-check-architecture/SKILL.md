...
grep -rL "'use server'" src/modules/*/actions/ --include="*.ts" 2>/dev/null || echo "PASS: All have directive"

echo "=== 6. error.tsx without 'use client' ==="
find app/ -name "error.tsx" 2>/dev/null | while read f; do
  grep -L "'use client'" "$f" 2>/dev/null
done || echo "PASS: All have directive"
...
grep -rn "dangerouslySetInnerHTML" src/ app/ --include="*.tsx" 2>/dev/null || echo "PASS: None"
...
grep -rn "getServerSideProps\|getStaticProps\|getStaticPaths" src/ app/ --include="*.ts" --include="*.tsx" 2>/dev/null || echo "PASS: None"
grep -rn "from.*next/router" src/ app/ --include="*.ts" --include="*.tsx" 2>/dev/null || echo "PASS: None"

echo "=== 17. <img> instead of next/image ==="
grep -rn "<img " src/ app/ --include="*.tsx" 2>/dev/null || echo "PASS: None"
...
done || echo "PASS: All have metadata"
```
