...
<template>
  <div class="ct">
    <!-- Version Navigation -->
      ...
      <a href="/catalog/v7" class="version-link active">V7</a>
    ...
    <!-- Header: V4 Terminal but compact — stats inline -->
    ...
    <Transition :name="transitionName" mode="out-in">
      ...
      <div v-if="view === 'list'" key="list" class="ct-view">
        <!-- Toolbar: compact terminal style -->
        ...
        <!-- Cards: V2 compact — single-line truncated, merged rows -->
        <div v-if="filtered.length" class="ct-grid">
          ...
          >
            ...
            <div class="card-meta-row">
              ...
              <span v-if="item.tools.length" class="meta-item">{{ item.tools.length }}t</span>
              <span :class="['meta-item', 'source-dot', item.source]">{{ item.source === 'official' ? t.official : t.community }}</span>
              <span class="meta-sep">|</span>
              <span class="meta-item author-inline">{{ item.author }}</span>
      ...
      <!-- DETAIL VIEW: V3 Showcase but tighter -->
      <div v-else-if="view === 'detail' && selected" key="detail" class="ct-view ct-detail" :style="{ '--c': selected.color }">
        ...
        <!-- Tighter gradient hero -->
        <div class="dt-hero">
          <div class="dt-hero-icon">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--c)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="dt-hero-badges">
            ...
            <span class="dt-badge cat">{{ catLabel(selected.category) }}</span>
          </div>
          <!-- Smaller command pill -->
          <div class="dt-cmd-pill">
            <code>{{ selected.copyCommand }}</code>
            <button class="dt-cmd-copy" :class="{ ok: detailCopied }" @click="copyCmd">
              <svg v-if="!detailCopied" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
        ...
        <!-- 2-column layout with narrower sidebar -->
        <div class="dt-layout">
          <main class="dt-main">
            <p class="dt-desc">{{ selected.description }}</p>

            <!-- Workflow as tighter stacked cards -->
            ...
            <div class="dt-meta-card">
              ...
              <div class="dt-meta-row">
                ...
                <span class="dt-meta-val dt-meta-cat">
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="var(--c)" stroke-width="2" v-html="catIcons[selected.category] || ''"></svg>
...
/* ═══ LAYOUT ═══ */
.ct { max-width: 1200px; margin: 0 auto; padding: 0 24px 48px; min-height: 80vh; overflow: hidden; }

/* ═══ HEADER — V4 Terminal compact, single-line with inline stats ═══ */
.ct-header { padding: 20px 0 14px; }
.ct-header-inner { display: flex; align-items: baseline; justify-content: space-between; gap: 16px; }
.ct-logo { display: flex; align-items: baseline; gap: 4px; }
.ct-logo-prompt { font-family: var(--vp-font-family-mono); font-size: 18px; font-weight: 700; color: var(--vp-c-brand-1); }
.ct-logo-text { font-family: var(--vp-font-family-mono); font-size: 22px; font-weight: 800; color: var(--vp-c-text-1); letter-spacing: -.02em; }
.ct-logo-cursor { font-family: var(--vp-font-family-mono); font-size: 22px; font-weight: 300; color: var(--vp-c-brand-1); animation: blink 1s step-end infinite; }
.ct-subtitle { font-size: 11px; color: var(--vp-c-text-3); margin: 4px 0 0; line-height: 1.4; font-family: var(--vp-font-family-mono); }
.ct-stats {
  display: flex; align-items: center; gap: 8px;
  font-family: var(--vp-font-family-mono); font-size: 11px; font-weight: 600; color: var(--vp-c-text-3);
}
.ct-stat-item { color: var(--vp-c-text-2); }
...
/* ═══ TOOLBAR — Compact V4 terminal ═══ */
.ct-toolbar { margin-bottom: 10px; display: flex; flex-direction: column; gap: 6px; }
.ct-search-box {
  display: flex; align-items: center; gap: 6px; padding: 6px 12px;
  border: 1px solid var(--vp-c-divider); border-radius: 4px;
  background: var(--vp-c-bg); color: var(--vp-c-text-3);
}
.ct-search-prompt { font-family: var(--vp-font-family-mono); font-weight: 700; color: var(--vp-c-brand-1); font-size: 12px; }
.ct-search-box input {
...
.ct-pills { display: flex; gap: 3px; flex-wrap: wrap; }
.pill {
...
.pill-sep {
  ...
  margin: 0 1px; font-size: 10px;
}
.ct-count { font-size: 10px; color: var(--vp-c-text-3); margin-bottom: 10px; font-family: var(--vp-font-family-mono); }

/* ═══ GRID ═══ */
.ct-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; }

/* ═══ CARD — V2 compact: no footer area, merged stats+tags+author ═══ */
.card {
...
.card-desc {
  font-size: 11px; color: var(--vp-c-text-2); line-height: 1.4;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
}

/* Merged meta row: stats + author + copy + tags in one line */
.card-meta-row {
...
.meta-sep { color: var(--vp-c-divider); font-size: 9px; font-family: var(--vp-font-family-mono); }
.source-dot::before {
...
.author-inline { font-weight: 500; color: var(--vp-c-text-3); }
.card-copy {
  padding: 2px 4px; border-radius: 3px; border: 1px solid var(--vp-c-divider);
...
/* ═══ EMPTY ═══ */
.ct-empty { text-align: center; padding: 40px 20px; color: var(--vp-c-text-2); font-family: var(--vp-font-family-mono); }
.ct-empty-msg { margin: 0 0 4px; font-size: 13px; }
.ct-empty-sub { font-size: 11px; color: var(--vp-c-text-3); margin: 0 0 12px; }

/* ═══════════════════════════════════════════ */
/* ═══ DETAIL VIEW — V3 Showcase tighter ═══ */
/* ═══════════════════════════════════════════ */
.dt-back {
  display: inline-flex; align-items: center; gap: 5px; padding: 4px 10px;
  cursor: pointer; color: var(--vp-c-text-3); margin: 6px 0 14px;
  transition: color .15s; font-size: 12px; font-weight: 500; border-radius: 6px;
}
...
.dt-hero {
  ...
  background: linear-gradient(135deg, color-mix(in srgb, var(--c) 16%, var(--vp-c-bg-soft)), color-mix(in srgb, var(--c) 5%, var(--vp-c-bg-soft)));
}
.dt-hero-icon {
  width: 52px; height: 52px; border-radius: 14px; background: var(--vp-c-bg);
  display: flex; align-items: center; justify-content: center;
  margin: 0 auto 12px; box-shadow: 0 3px 12px rgba(0,0,0,0.07); border: 1px solid var(--vp-c-divider);
}
.dt-hero-title { font-size: 26px; font-weight: 800; color: var(--vp-c-text-1); margin: 0 0 10px; font-family: var(--vp-font-family-mono); }
.dt-hero-badges { display: flex; justify-content: center; gap: 5px; margin-bottom: 16px; }
.dt-badge {
  display: inline-flex; align-items: center; gap: 3px; font-size: 9px; font-weight: 700;
  padding: 2px 8px; border-radius: 5px; text-transform: uppercase; letter-spacing: .4px;
}
.dt-badge.agent {
...
/* Smaller command pill */
.dt-cmd-pill {
  display: inline-flex; align-items: center; gap: 10px; max-width: 420px;
  margin: 0 auto; padding: 8px 8px 8px 16px;
  background: var(--vp-c-bg); border: 1px solid var(--vp-c-divider); border-radius: 20px;
  box-shadow: 0 1px 8px rgba(0,0,0,0.05);
}
.dt-cmd-pill code { font-size: 13px; font-weight: 600; color: var(--vp-c-text-1); font-family: var(--vp-font-family-mono); }
.dt-cmd-copy {
...
/* 2-col layout with narrower sidebar */
.dt-layout { display: grid; grid-template-columns: 1fr 220px; gap: 28px; }
.dt-main { min-width: 0; }
.dt-desc { font-size: 13px; line-height: 1.65; color: var(--vp-c-text-2); margin: 0 0 24px; }

/* Sections with colored dot */
.dt-section { margin-bottom: 24px; }
.dt-section-title {
  display: flex; align-items: center; gap: 6px;
  font-size: 14px; font-weight: 700; color: var(--vp-c-text-1); margin: 0 0 12px;
  padding-bottom: 8px; border-bottom: 1px solid var(--vp-c-divider);
}
.dt-dot { width: 5px; height: 5px; border-radius: 50%; background: var(--c); flex-shrink: 0; }

/* Tighter workflow stacked cards */
.dt-wf-cards { display: flex; flex-direction: column; gap: 0; }
.dt-wf-card {
  display: flex; align-items: flex-start; gap: 10px; position: relative;
  padding: 10px 12px; background: var(--vp-c-bg-soft); border: 1px solid var(--vp-c-divider);
  border-radius: 8px;
}
.dt-wf-card + .dt-wf-card { margin-top: 10px; }
.dt-wf-num {
...
.dt-wf-name { font-size: 12px; font-weight: 700; color: var(--vp-c-text-1); }
.dt-wf-steps { margin: 4px 0 0; padding: 0 0 0 12px; list-style: none; }
.dt-wf-steps li {
...
.dt-wf-connector {
  position: absolute; left: 22px; bottom: -11px;
  width: 2px; height: 10px; background: var(--c); opacity: .3;
}

/* Prose at 13px */
.dt-prose { font-size: 13px; line-height: 1.6; color: var(--vp-c-text-2); }
.dt-prose :deep(.fc-h3) { font-weight: 700; color: var(--vp-c-text-1); margin: 10px 0 3px; font-size: 13px; }
.dt-prose :deep(.fc-li) { display: flex; align-items: baseline; gap: 5px; margin: 2px 0; }
.dt-prose :deep(.fc-num) { font-weight: 600; color: var(--vp-c-text-3); min-width: 16px; }
.dt-prose :deep(.fc-dot) { width: 3px; height: 3px; border-radius: 50%; background: var(--vp-c-text-3); flex-shrink: 0; margin-top: 7px; }
...
.dt-tip code {
  ...
  font-size: 12px; font-family: var(--vp-font-family-mono); line-height: 1.5;
}

/* Sidebar with colored left accent */
.dt-sidebar { position: sticky; top: 80px; align-self: start; display: flex; flex-direction: column; gap: 10px; }
.dt-meta-card {
...
.dt-meta-row {
  ...
  padding: 8px 12px; border-bottom: 1px solid var(--vp-c-divider); font-size: 11px;
}
...
.dt-meta-cat { display: flex; align-items: center; gap: 3px; }

.dt-tools-card { border: 1px solid var(--vp-c-divider); border-radius: 8px; padding: 10px 12px; }
.dt-tools-title { font-size: 9px; font-weight: 700; color: var(--vp-c-text-3); text-transform: uppercase; letter-spacing: .5px; margin: 0 0 6px; }
.dt-tools-list { display: flex; flex-wrap: wrap; gap: 3px; }
.dt-tool {
...
.dt-github-btn {
  ...
  padding: 8px; border-radius: 8px; border: 1px solid var(--vp-c-divider);
  background: var(--vp-c-bg-soft); color: var(--vp-c-text-2); font-size: 12px; font-weight: 600;
...
@media (max-width: 960px) {
  ...
  .dt-sidebar { position: static; flex-direction: row; flex-wrap: wrap; gap: 8px; }
  .dt-meta-card, .dt-tools-card { flex: 1; min-width: 180px; }
}
@media (max-width: 640px) {
  .ct-header-inner { flex-direction: column; align-items: flex-start; gap: 8px; }
  .ct-header { padding: 16px 0 12px; }
  .ct-grid { grid-template-columns: 1fr; }
  .dt-hero { padding: 20px 14px 18px; }
  .dt-hero-title { font-size: 20px; }
  .dt-cmd-pill { flex-direction: column; gap: 6px; padding: 10px; border-radius: 12px; }
}
